...
def load_encoder_weight(
    tensorrt_llm_whisper, model_metadata: dict, model_params: dict, n_layer: int
):
...
def load_decoder_weight(
    ...
    for i in range(tllm_model.num_layers):
        ...
        layer.self_attention.qkv.weight.value = fuse_qkv(
            trans_weight(
                model_params["decoder.blocks." + str(i) + ".attn.query.weight"].numpy()
            ),
            trans_weight(
                model_params["decoder.blocks." + str(i) + ".attn.key.weight"].numpy()
            ),
            trans_weight(
                model_params["decoder.blocks." + str(i) + ".attn.value.weight"].numpy()
            ),
        )
        ...
        cross_attn_qkv_weight = fuse_qkv(
            trans_weight(
                model_params[
                    "decoder.blocks." + str(i) + ".cross_attn.query.weight"
                ].numpy()
            ),
            trans_weight(
                model_params[
                    "decoder.blocks." + str(i) + ".cross_attn.key.weight"
                ].numpy()
            ),
            trans_weight(
                model_params[
                    "decoder.blocks." + str(i) + ".cross_attn.value.weight"
                ].numpy()
            ),
        )
